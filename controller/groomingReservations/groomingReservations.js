...
router.post("/grooming-reservations/create", authenticateToken, async (req, res) => {
  try {
...
router.get("/grooming-reservations", authenticateToken, async (req, res) => {
  try {
    const { data, error } = await supabase.from("grooming_reservations").select("*");
...
router.get("/grooming-reservations/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;

    const { data, error } = await supabase.from("grooming_reservations").select("*").eq("grooming_reservations_id", id).single();
...
router.put("/grooming-reservations/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { transaction_id, price, schedule, service } = req.body;
    ...
    const { data, error } = await supabase.from("grooming_reservations").update(updates).eq("grooming_reservations_id", id).select("*");
...
router.delete("/grooming-reservations/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;

    const { data, error } = await supabase.from("grooming_reservations").delete().eq("grooming_reservations_id", id).select("*");
