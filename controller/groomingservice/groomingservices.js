...
router.get("/grooming-service", authenticateToken, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "An error occurred while fetching grooming services.",
      error: error.message,
    });
  }
});
...
router.get("/grooming-service/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (error || !data) {
      return res
        .status(404)
        .json({ success: false, message: "Grooming service not found." });
    }
    ...
    return res.status(500).json({
      success: false,
      message: "An error occurred while fetching grooming service.",
      error: error.message,
    });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (error || !data.length) {
        return res.status(400).json({
          success: false,
          message: "Failed to update grooming service.",
        });
      }
      ...
      return res.status(500).json({
        success: false,
        message: "An error occurred while updating grooming service.",
        error: error.message,
      });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        return res.status(400).json({
          success: false,
          message: "Failed to delete grooming service.",
        });
      }
      ...
      return res.status(500).json({
        success: false,
        message: "An error occurred while deleting grooming service.",
        error: error.message,
      });
    }
  }
